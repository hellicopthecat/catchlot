...
func (db *UserRepo) RUpdateUser(ctx context.Context, req request.UpdateUserRequest) error {
	q, err := os.ReadFile(constants.UpdateSQL + "u_users.sql")
	if err != nil {
		commons.BadSQLFile(err)
		return err
	}
	result, err := db.db.Exec(string(q), req.Nickname, req.RefreshToken, req.Email)
	if err != nil {
	...
	if affected == 0 {
		...
		return fmt.Errorf("해당 유저가 존재하지 않음. :: %s ", req.Email)
	}
	if err != nil {
